...
backupfolder=USER@HOST:/backup/path
sshremoteport=22 #default ssh port is 22 but use the one in use for the host

emailrecipient="me@example.com, collegue@example.com"

#Debut du Script

if [ -s /var/log/postbackup.log ]; then
  rm /var/log/postbackup.log
fi

exec 2>>/var/log/postbackup.log

for paths in $(cat /tmp/path-to-backup.list); do
